...
Dashboard consolidation and data-integrity baseline are now in effect:
- Dashboard serves a single React UI build from `KubeAI/dashboard/static`.
- Legacy static dashboard assets are replaced by React build output.
- Runtime startup uses real state only (no fake seeded state).
- Default MCP auto-registration is removed; MCPs must be explicitly registered.
- Model assignment enforces provider credential presence for non-local models.
- Deployment specs permit `min_replicas = 0` for clean empty startup.
